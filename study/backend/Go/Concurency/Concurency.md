---
tags: [go, concurrency, patterns, backend]
---
...
- **Конкурентность (Concurrency)** — это про **структуру** программы. Это способ организации кода, который позволяет ему работать с несколькими задачами _одновременно_ в нашем мышлении.
    
- **Параллелизм (Parallelism)** — это про **выполнение** программы. Это когда несколько задач выполняются _буквально в один и тот же момент времени_.

**Аналогия: Шеф-повар на одной плите (одно ядро CPU)**
Представьте одного повара, который готовит два блюда:

1. Он ставит варить пасту.
    
2. Пока паста варится, он не ждёт у плиты, а нарезает овощи для салата.
    
3. Когда паста сварилась, он возвращается к ней, сливает воду, и пока она остывает, заправляет салат.

**Аналогия: Два шеф-повара на одной кухне (два ядра CPU)**  
Теперь два блюда готовят два повара:

- Первый повар варит пасту.
    
- Второй повар одновременно нарезает салат.
...
## Related
- [[Goroutine]] — фундаментал горутин
- [[Планировщик (Scheduler)]] — как горутины мультиплексируются на потоки
- [[Каналы (Channels) Внутреннее устройство|Channels]] — внутреннее устройство каналов (hchan, lock, очереди)
- [[Context]] — контекст для отмены в pipeline и worker pool
