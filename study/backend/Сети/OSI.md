...
- Обеспечивает интерфейс между сетевыми службами и приложениями
    
- Предоставляет услуги непосредственно пользовательским приложениям
    
- Отвечает за передачу файлов, обмен сообщениями, удаленный доступ
    

**Примеры протоколов:**

- HTTP/HTTPS - веб-браузинг
...
- Преобразование данных в единый формат для передачи
    
- Сжатие данных для уменьшения объема
...
- ASCII, Unicode - кодировки символов
    
- JPEG, MPEG - форматы сжатия
...
- Контроль диалога между системами
    
- Восстановление сеансов после сбоев
...
- Обеспечение надежной end-to-end передачи данных
    
- Сегментация данных и их повторная сборка
...
- SCTP - передача потоковых данных
...
- Определение оптимальных путей передачи
...
- ICMP - управляющие сообщения
    
- BGP, OSPF, RIP - протоколы маршрутизации
    
- ARP - преобразование адресов

### 6. Канальный уровень (Data Link Layer)

**Назначение:**

- Передача кадров между узлами в одной сети
    
- Обнаружение и исправление ошибок на уровне кадра
...
- Ethernet - проводные LAN сети
    
- Wi-Fi (IEEE 802.11) - беспроводные сети
    
- PPP - точка-точка соединения
...
- Физическая передача битов по каналу связи
    
- Определение электрических, механических и функциональных характеристик
    
- Кодирование сигналов
    
- Передача неструктурированных raw-битов
...
## Related
- [[TCP & UDP]] — транспортный уровень (L4) в деталях
- [[HTTP]] — протокол прикладного уровня (L7)
- [[API]] — REST и gRPC работают на прикладном уровне
- [[Как работает браузер]] — обход всех уровней OSI при браузерном запросе
